    ...
    >
      <v-card flat class="rounded-lg">
        ...
        <v-card dark flat color="transparent" class="px-8 pt-6">
          <v-row no-gutters>
            <v-col cols="10">
              <div class="text-h5 font-weight-thin">Add Product</div>
            </v-col>
  ...
  methods: {
    AddProduct() {
      ...
      this.promiseFetch(this.$store.getters.fetchTimeout)(
        fetch(`${this.$store.getters.endpoint}Products/Add/`, {
          method: "post",
          body: JSON.stringify({
        ...
        .then((response) => {
          response.text().then((res) => {
            if (response.status > 200) {
              ...
              this.$emit("NewProduct", JSON.parse(res));
              this.visible = false;
            }
          });
        })
